...
function PasswordStrength({ password }) {
  ...
  const bars   = ['', 'bg-red-500', 'bg-orange-500', 'bg-yellow-500', 'bg-emerald-500']
  ...
  return (
    <div className="mt-2 space-y-2">
      <div className="flex gap-1 items-center">
        {[1,2,3,4].map((i) => (
          <div key={i} className={`h-1 flex-1 rounded-full transition-all duration-300 ${i <= score ? bars[score] : 'bg-gray-700'}`} />
        ))}
...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Send OTP via EmailJS to the user's inbox
      if (result.otp) {
        try {
          ...
          await emailjs.send(
            ...
            import.meta.env.VITE_EMAILJS_TEMPLATE_ID,
            {
              ...
              to_name:       form.fullName.trim(),   // matches {{to_name}} in template
              otp_code:      String(result.otp),     // matches {{otp_code}} in template
              valid_minutes: '5',                    // matches {{valid_minutes}} in template
            }
          )
        } catch (emailErr) {
          // Show a warning but still let user proceed to OTP screen
          setServerError('Account created but OTP email failed to send. Please check your EmailJS template configuration.')
          console.error('[EmailJS] Failed to send registration OTP:', emailErr)
          setLoading(false)
          return
        }
      }
        ...
        <div className="rounded-2xl border border-gray-800 bg-gray-900/80 backdrop-blur-sm p-8 shadow-2xl shadow-black/40">
          ...
          <form onSubmit={handleSubmit} className="space-y-5" noValidate>
            ...
            <div>
              ...
              <input name="fullName" type="text" value={form.fullName} onChange={handleChange} placeholder="John Smith" autoComplete="name"
                className={`w-full rounded-xl bg-gray-800 border px-4 py-3 text-sm text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all ${errors.fullName ? 'border-red-500/70 focus:ring-red-500' : 'border-gray-700'}`}
              />
            ...
            {/* Role */}
            <div>
              <label className="block text-xs font-semibold text-gray-400 mb-1.5 uppercase tracking-wider">Role / Department</label>
              <select name="role" value={form.role} onChange={handleChange}
                className={`w-full rounded-xl bg-gray-800 border px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all cursor-pointer appearance-none ${form.role ? 'text-white' : 'text-gray-500'} ${errors.role ? 'border-red-500/70 focus:ring-red-500' : 'border-gray-700'}`}
              >
            ...
            <div>
              ...
              <div className="relative">
                <input name="password" type={showPassword ? 'text' : 'password'} value={form.password} onChange={handleChange} placeholder="Min. 8 characters" autoComplete="new-password"
            ...
            <div>
              ...
              <div className="relative">
                <input name="confirmPassword" type={showConfirm ? 'text' : 'password'} value={form.confirmPassword} onChange={handleChange} placeholder="Re-enter your password" autoComplete="new-password"
                  className={`w-full rounded-xl bg-gray-800 border px-4 py-3 pr-11 text-sm text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all ${errors.confirmPassword ? 'border-red-500/70 focus:ring-red-500' : form.confirmPassword && form.confirmPassword === form.password ? 'border-emerald-500/70' : 'border-gray-700'}`}
                />
                <button type="button" onClick={() => setShowConfirm((p) => !p)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-200 transition-colors" tabIndex={-1}>
            ...
            {/* Terms */}
            <div>
              <label className="flex items-start gap-3 cursor-pointer select-none group" onClick={() => { setAgreed((p) => !p); if (errors.agreed) setErrors((p) => ({ ...p, agreed: '' })) }}>
                <div className={`mt-0.5 w-4 h-4 rounded border shrink-0 flex items-center justify-center transition-all ${agreed ? 'bg-indigo-600 border-indigo-600' : 'bg-gray-800 border-gray-600 group-hover:border-gray-400'}`}>
                ...
                <span className="text-sm text-gray-400 group-hover:text-gray-300 transition-colors leading-tight">
                  I agree to the <span className="text-indigo-400 hover:text-indigo-300 underline underline-offset-2">Terms of Service</span> and <span className="text-indigo-400 hover:text-indigo-300 underline underline-offset-2">Privacy Policy</span>
                </span>
              </label>
            ...
            <p className="text-xs text-gray-500 flex items-center gap-1.5">
              ...
              A verification OTP will be sent to your email address.
            </p>
          ...
          <Link to="/login" className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl border border-gray-700 text-sm text-gray-300 hover:text-white hover:border-gray-500 hover:bg-gray-800/50 transition-all duration-200 font-medium">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
